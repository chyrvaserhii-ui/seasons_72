...
/// One-line inline display of a kō's date range, tappable for full
/// calendar context.
///
/// Replaces the prior diverging treatments:
///   • Home screen: a bare "5 квітня – 9 квітня" date row with a dot
///   • Detail screen: a full bordered PeriodCard with its own modal
///
/// Visually editorial-light — no card border, no per-kō tint — the
/// engraving's "кō N з 72" badge already carries the position counter,
/// so this row stays minimal: 📅 + date range. Tapping it opens the
/// same details sheet the old PeriodCard had: hero range, "5 days",
/// and three position blocks (in the year / phase / season).
///
/// Format:  📅  5 квітня – 9 квітня   →  tap to open details
class PeriodStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Padding(
          // Horizontal 14 mirrors the inner padding all 8 cultural
          // cards below use, so the trailing info icon lines up in a
          // straight vertical column with the cards' info icons. Icon
          // size + alpha bumped to 18 / 0.45 for the same column-rhythm
          // reason.
  ...
  void _showSheet(BuildContext context, bool isUk) {
    // Inkstone slate — the same neutral accent the deleted PeriodCard
    // used. Kept independent of the kō's meta-season so the period
    // sheet has its own colour identity.
